...
use crate::{PacketHash, PacketPathHash};
...
pub fn decode(
    ...
    let packet_value = crate::ucs03_zkgm_0::packet::decode(packet).context("decode packet")?;

    let ack_value_by_path = match ack {
        Some(ack) => {
            find_acks(&mut crate::ucs03_zkgm_0::ack::decode(packet, ack).context("decode ack")?)?
        }
        None => HashMap::new(),
    };
